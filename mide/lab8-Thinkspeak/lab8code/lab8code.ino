...
void setup() {
    Serial.begin(115200); //ไม่มีก็ได้แต่เอาไว้ดู ในคอมถึงจะดูได้
    delay(10);
    pinMode(PUMP_RLY, OUTPUT); // Initialize the PUMP_RLY(4) pin as an output //ขาข้อมูลที่ส่งไปที่ ดีเลย์
...
void loop() {
    ...
    readDht(dht, &t_dht, &h_dht); //อ่านค่า คำสั่งนี้อยู่ใน dht.h
    if(t_dht > 40) // condition for make relay on
    {
    ...
    uploadThingsSpeak(t_dht, h_dht); //คำสั่งให้เอาข้อมูลขึ้น thingspeak
...
void uploadThingsSpeak(float t, float h) {
    ...
    static const char* apiKey = "Q9L7X51LRP8J5DJU"; ///////////////////////
